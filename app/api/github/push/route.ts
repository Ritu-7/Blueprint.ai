  ...
  try {
    ...
    if (!token || !username) {
      return NextResponse.json(
        { success: false, error: 'GitHub credentials not configured in environment' },
        { status: 500 }
      );
    }
    ...
    if (!createRepoRes.ok) {
      ...
      console.error('[github] repository creation failed', { status: createRepoRes.status, body: errorBody });
      
      // If it exists, proceed with push
      if (errorBody.errors?.some((e: any) => e.message === 'name already exists on this account')) {
    ...
    for (const file of files) {
      ...
      if (!blobRes.ok) {
        ...
        console.error(`[github] Failed to create blob for ${file.path}`, { status: blobRes.status, body: errorBody });
    ...
    if (!treeRes.ok) {
      ...
      console.error('[github] Failed to create tree', { status: treeRes.status, body: errorBody });
    ...
    if (!commitRes.ok) {
      ...
      console.error('[github] Failed to create commit', { status: commitRes.status, body: errorBody });
    ...
    console.log(`[github] Updating ref: refs/heads/${branchName}`);
    ...
    if (!createRefRes.ok) {
      ...
      if (!updateRefRes.ok) {
        ...
        console.error('[github] Failed to update ref', { status: updateRefRes.status, body: errorBody });
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to push to GitHub' },
      { status: 500 }
    );
  }
}
