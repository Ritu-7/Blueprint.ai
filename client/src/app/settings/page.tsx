  ...
  return (
    <WorkspacePage eyebrow="Workspace controls" title="Settings" description="Configure your Blueprint workspace and the integrations that support your build loop." icon={Settings2}>
      <div className="grid gap-4 md:grid-cols-2">
        {settings.map(({ icon: Icon, title, detail }) => <button type="button" key={title} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-left transition hover:border-cyan-400/40 hover:bg-white/[0.06]"><Icon className="h-5 w-5 text-cyan-300" /><h2 className="mt-6 text-xl font-black">{title}</h2><p className="mt-3 text-sm leading-6 text-white/50">{detail}</p><span className="mt-6 block text-xs font-black uppercase tracking-[0.18em] text-cyan-200 opacity-70 group-hover:opacity-100">Configure</span></button>)}
      </div>
    </WorkspacePage>
  );
}
